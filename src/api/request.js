...
router.get("/:requestId/users", (req, res, next) => {
    ...
    requestTable.getRequestUsers(requestId)
        .then(({ users }) => {
            res.json(users);
        })
        .catch((error) => next(error))
...
router.get("/current", (req, res, next) => {
    ...
    accountHelper.getCurrentUser(token)
        .then((result) => {
            if (result.user) {
                let currentUser = result.user

                requestTable.getByUserId(currentUser.id)
                    .then((result) => {
                        res.json(result)
                    })
                    .catch((error) => next(error))
...
router.post("/reply", (req, res, next) => {
    ...
    if (request_id === '' || request_id === null || request_id === undefined || request_id < 0) {
        ...
        accountHelper.getCurrentUser(token)
            .then((result) => {
                if (result.user) {
                    ...
                    requestTable.insertUserRequest({ content, request_id, creator })
                        .then((result) => {
            ...
            accountHelper.getCurrentUser(token)
                .then((result) => {
                    if (result.user) {
                        ...
                        requestTable.insert({content, creator})
                            .then(({r}) => {
                                requestTable.getLastRequest()
                                    .then((result) => {
                                        if (result.length > 0) {
            ...
            accountHelper.getCurrentUser(token)
                .then((result) => {
                    if (result.user) {
                        ...
                        if (errors.length === 0) {
                            requestTable.update({ id, content, creator, users_data })
                                .then(({ result }) => {
                                    if (result.success) {
