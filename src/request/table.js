...
class RequestTable {
  static index() {
    return new Promise((resolve, reject) => {
      pool.query(
        "SELECT requests.*, json_agg(json_build_object('id', users.id, 'name', users.name)::jsonb) as users\n" +
          "FROM requests\n" +
  ...
  static getByUserId(userId) {
    return new Promise((resolve, reject) => {
      pool.query(
        "select requests.*, json_agg(json_build_object('id', rr.id, 'content', rr.content, 'created_at', rr.created_at)::jsonb) as replies\n" +
          "from requests\n" +
          "left join request_replies as rr on requests.id = rr.request_id \n" +
          "where cast(rr.creator->>'id' as integer) = " +
          userId +
          "\n" +
          "group by requests.id",
        [],
  ...
  static getUserRequest(requestIds) {
    return new Promise((resolve, reject) => {
      pool.query(
        `SELECT * FROM requests WHERE id IN(${requestIds})`,
        [],
        (error, result) => {
          if (error) return reject(error)
  ...
  static insertUserRequest({ content, request_id, creator }) {
    return new Promise((resolve, reject) => {
      let id = request_id
      globalQuery.checkExist({ id, table_name }).then((data) => {
        if (data.length > 0) {
          ...
          resolve({
            result: { success: false, message: id + " ID-тай хүсэлт олдсонгүй" },
          })
        }
      })
    })
  ...
  static insertRequestReply({ content, request_id, creator }) {
    return new Promise((resolve, reject) => {
      pool.query(
        "INSERT INTO request_replies(content, request_id, creator) VALUES ($1, $2, $3)",
        [content, request_id, creator],
        (error, result) => {
          if (error) return reject(error)
  ...
  static insert({ content, creator }) {
    return new Promise((resolve, reject) => {
      pool.query(
        `INSERT INTO requests(content, creator) VALUES($1, $2)`,
        [content, creator],
        (error, result) => {
          if (error) return reject(error)
          resolve({ result: { success: true, message: "Хүсэлтийг хадгаллаа" } })
  ...
  static insertUser({ data }) {
    return new Promise((resolve, reject) => {
      pool.query(
        format("INSERT INTO request_users(request_id, user_id) VALUES %L", data),
        [],
        (error, response) => {
          if (error)
            return reject({
              success: false,
              message: "Хүсэлтийг устгаж чадсангүй",
              error: error,
            })
  ...
  static update({ id, content, creator, users_data }) {
    return new Promise((resolve, reject) => {
      globalQuery.checkExist({ id, table_name }).then((data) => {
        if (data.length > 0) {
          pool.query(
            `UPDATE requests SET content = $1, creator = $2 WHERE id = $3`,
            [content, creator, id],
            (error, result) => {
              if (error) return reject(error)
              ...
              resolve({
                result: { success: true, message: id + " ID-тай хүсэлтийг заслаа" },
              })
            }
          )
        } else {
                      ...
                      if (result.success) {
                        pool.query(
                          ...
                          (error, result) => {
                            ...
                            resolve({
                              result: { success: true, message: "Хүсэлтийг устгалаа" },
                            })
                          }
                        )
                      }
  ...
  static deleteUsers({ id }) {
    return new Promise((resolve, reject) => {
      pool.query(
        "DELETE FROM request_users WHERE request_id = $1",
        [id],
        (error, result) => {
          if (error) return reject(error)
  ...
  static deleteReplies({ id }) {
    return new Promise((resolve, reject) => {
      pool.query(
        "DELETE FROM request_replies WHERE request_id = $1",
        [id],
        (error, result) => {
          if (error) return reject(error)
