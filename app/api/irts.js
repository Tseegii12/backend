        ...
        IrtsTable.getAllDistinct()
          .then((response1) => {
            response1.forEach((item, index) => {
              IrtsTable.getAllCountDistinct(item.manager_id).then((response2) => {
                let innerArray = []
              ...
              IrtsTable.getAllCountDistinctDate(item.manager_id).then((res4) => {
                let innerArray = []
                innerArray.push(item.manager_id)
                innerArray.push(item.manager_name)
                innerArray.push(item.pj_name)
...
router.post("/getBySearch", (req, res, next) => {
  ...
  let arrHusnegtData = []
  let arrHusnegtHeader = ["#", "Төсөл", "Овог", "Нэр", "Албан тушаал", "Өдөр", "Төлөв"]
